        ...
        for i in range(1, n+1) :
            for j in range(1, required+1) :
                ...
                if nums[i-1] <= j :
                    ...
                    if option[0] :
                        if table[i][j][0] :
                            table[i][j] = (True, max(1 + option[1], table[i][j][1]))
                        else :
                            table[i][j] = (True, (1 + option[1]))
        
        # Find any "True" value with the required average
        for i in range(1, n+1) :
            for j in range(1, required+1) :
                if table[i][j][0] :
                    cur_val = j / table[i][j][1]
                    if cur_val == avg and table[i][j][1] < n :
                        return True
...
ob = Solution()
print(ob.splitArraySameAverage([3,4,9,4,4,3,9,8,5,3]))
